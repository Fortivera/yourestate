...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g.selectAll<SVGGElement, d3.PieArcDatum<[string, number]>>('g')
            .data(pieData)
            .enter()
            .append('g')
            .on('click', function (_, j) {
                ...
                // Reset all slices to default color
                slice.select('path')
                    .transition()
                    .duration(200)
                ...
                // If clicked, keep the clicked slice fully colored; otherwise, dim it
                (clickedSlice.select('path').node() as SVGPathElement)
                    .transition()
                    .duration(200)
                    .attr('fill', () => (isClicked ? String(color(j.toString())) : d3.color(color(j.toString()))?.darker(0.3).toString()));
