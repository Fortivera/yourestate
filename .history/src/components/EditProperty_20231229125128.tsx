...
export default function EditProperty({ property, allProperties: cachedProperties }: Props) {
    ...
    const mutation = useMutation((newPropertyData: Property) => updateProperty(newPropertyData, property.id), {
        onSuccess: (data: any, variables: Property) => {
            // Assuming the server response with the updated property
            const updatedProperty = variables
            // Update the cached data
            const updatedCachedProperties = cachedProperties!.map((p) => (p.id === updatedProperty.id ? updatedProperty : p))
