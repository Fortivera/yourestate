...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        const g1 = svg.append("g").attr("transform", `translate(${width / 4}, ${height / 2})`)

        const g2 = svg.append("g").attr("transform", `translate(${(width * 3) / 4}, ${height / 2})`)
        ...
        const handleSliceClick = (g: d3.Selection<SVGGElement, any, null, undefined>) => {
            const slice = g
                ...
                .on("click", function (event: d3.BaseType, j: number) {
