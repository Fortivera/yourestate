...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createBarChart = (data: Property[]) => {
        ...
        const energyTypes = ["gas", "hydro", "electricity"]
        const x1Scale = d3.scaleBand().domain(energyTypes).rangeRound([0, x0Scale.bandwidth()]).padding(0.05)

        // Compute the maximum value for the y-axis
        const maxEnergyValue = d3.max(data, ([, EnergySums]) => Math.max(energySums.gas, energySums.hydro, energySums.electricity))

        // Scale for the y-axis
        const yScale = d3
            .scaleLinear()
            .domain([0, maxEnergyValue])
