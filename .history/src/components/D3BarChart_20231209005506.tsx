...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createBarChart = (data: Property[]) => {
        ...
        const colorScale = d3.scaleOrdinal().domain(["electricity", "gas", "hydro"]).range(["#ffd1dc", "#c4edde", "#d3a3e1"]) // pastel colors

        const types = ["electricity", "gas", "hydro"]

        types.forEach((type: keyof EnergySums, i) => {
            svg.selectAll(`.${type}`)
                .data(energyTuples)
                .enter()
                .append("rect")
                ...
                .attr("height", ([, energySums]) => 500 - yScale(energySums[type as keyof EnergySums]))

                .attr("fill", colorScale(energ))
        })

        const xAxis = d3.axisBottom(xScale).tickSizeOuter(0)
        const yAxis = d3.axisLeft(yScale)

        svg.append("g").attr("transform", `translate(0,${500})`).call(xAxis).append("text").attr("x", 500).attr("y", -20).attr("fill", "black").attr("text-anchor", "end").text("Country")

        svg.append("g").call(yAxis).append("text").attr("x", 20).attr("y", 0).attr("fill", "black").attr("text-anchor", "start").text("Amount")
