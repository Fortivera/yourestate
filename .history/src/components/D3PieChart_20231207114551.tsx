...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        function updateLegendColor() {
            legend.select("text").style("fill", theme === "light" ? "red" : "white")
        }

        // Call this function initially to set the color based on the initial theme
        updateLegendColor()
    ...
    return (
        <div className={`${theme === "light" ? "bg-slate-50" : "bg-slate-600 text-white"} w-full h-screen`}>
