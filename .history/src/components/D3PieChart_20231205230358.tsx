...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        let clickedIndex = -1 // Add this line
        ...
        const slice = g
            ...
            .on("click", function (_, d) {
                clickedIndex = clickedIndex === d.index ? -1 : d.index // Update this line

                // Brighten the clicked slice and dim out the other slices
                slice
                    .select("path")
                    .transition()
                    .duration(200)
                    .attr("fill", (_, k) => {
                        if (clickedIndex === k) {
                            return (d3.color(color(k.toString()))?.brighter(0.3) as any) || "#ccc"
                        } else {
