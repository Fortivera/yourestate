...
export const D3DonutPieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[], theme: string) => {
        ...
        const foreignObject = svg.append("foreignObject").attr("x", legendX).attr("y", legendY).attr("width", legendWidth).attr("height", legendHeight)

        // Create a div inside the foreignObject
        const legendDiv = foreignObject.append("xhtml:div").style("width", "100%").style("height", "100%").style("overflow-y", "scroll")

        // Create the legend inside the div
        ...
        // Append colored squares to the legend
        legend
            .append("svg")
            .attr("width", 20)
            .attr("height", 20)
