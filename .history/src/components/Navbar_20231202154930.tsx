  ...
  return (
    <>
      ...
      <div className={`md:hidden ${menuOpen ? "flex" : "hidden"} relative w-full z-50 transition-all duration-500 ease-in-out`}>
        <div className={`flex flex-col absolute right-0 border border-t-[#ddd] border-t-[0.1px] shadow-lg rounded-b-lg overflow-hidden backdrop-filter backdrop-blur-md ${theme === "light" ? "bg-gray-50 bg-opacity-50 text-gray-700" : "bg-gray-600 bg-opacity-50 text-gray-300"} p-1`}>
          <div className="px-3">
            <ThemeToggle />
          </div>
          <Link href={"dashboard/createproperty"}>
            <div className={`h-10 flex items-center px-3 ${theme === "light" ? "text-blue-500 hover:bg-slate-500 " : "text-blue-300"}`}>
              <SvgAddProperty className="m-1" strokeWidth={1.5} />
              <div>Add Property</div>
            </div>
          </Link>
