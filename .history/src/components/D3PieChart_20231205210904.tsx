    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g.selectAll('g')
            ...
            .on('click', function (event, j) {
                ...
                // If clicked, keep the clicked slice unchanged; otherwise, change the color
                const fillColor = isClicked ? String(color(j.toString())) : d3.color(color(j.toString()))?.darker(0.3).toString();

                // Apply color to slices based on whether clicked or not
                slice.select('path')
                    .transition()
                    .duration(200)
                    .attr('fill', (_, k) => (isClicked || String(k) === String(j) ? String(color(k.toString())) : fillColor || '#ccc'));
            });
