...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        const containerWidth = ref.current?.parentElement?.offsetWidth || 550;
        const containerHeight = ref.current?.parentElement?.offsetHeight || 500;

        // Set a breakpoint for mobile view
        const isMobileView = containerWidth <= 600;

        const width = isMobileView ? containerWidth : 550;
        const height = isMobileView ? containerHeight : 500;

        const svg = d3.select(ref.current)
            .attr('viewBox', isMobileView ? `0 0 ${containerWidth} ${containerHeight}` : `0 0 500 500`)
            .attr('preserveAspectRatio', isMobileView ? 'xMidYMid meet' : null);
    ...
    return <svg ref={ref} width={570} height={500} />;
};
