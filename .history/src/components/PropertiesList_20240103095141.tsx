            ...
            <div className={`w-full h-full pt-1 border-b-2 md:border-r-2 ${theme === "light" ? "bg-slate-600/10 border-[#e4e7ec]" : "propertyListDark border-[#536079]"}    shadow-neutral-400 shadow-sm overflow-auto`}>
                <div className="flex mx-4 font-Noto text-base  ">
                    <div className="flex w-full relative">
                        <input type="text" value={searchedData} className={`${theme === "light" ? "bg-slate-50" : "bg-slate-500/60"} relative pl-2 outline-none border-[1px] border-slate-200 rounded-lg mt-1 mb-1 h-8 w-full`} placeholder="Search..." onChange={handleChange} />
                        {searchedData && (
                            <button
                                onClick={handleSearchClear}
                                className={`${theme === "light" ? "text-slate-700 hover:bg-slate-200" : "text-white hover:bg-slate-400"}
                                    absolute right-6 top-1/2 transform -translate-y-1/2 bg-transparent rounded-full p-1`}
                            >
