...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createBarChart = (data: Property[]) => {
        ...
        svg.append("g").attr("class", "x axis").attr("transform", `translate(0,${height})`).call(xAxis).append("text").attr("x", 500).attr("y", 20).attr("fill", "black").attr("text-anchor", "end").text("Country")

        svg.append("g").attr("class", "y axis").call(yAxis).append("text").attr("x", 20).attr("y", 0).attr("fill", "black").attr("text-anchor", "start").text("Amount")
    ...
    return (
        <div className={`${theme === "light" ? "bg-slate-50" : "bg-[#515F73]"} w-full`}>
            <div className="max-w-[42.5rem] mx-auto p-2">
                <svg ref={ref} style={{ width: width + margin.left + margin.right, height: height + margin.top + margin.bottom }} />
            </div>
        </div>
    )
}
