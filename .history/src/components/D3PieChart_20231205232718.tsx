...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g
            ...
            .attr("data-slice-id", (d, i) => i.toString()) // Assign a unique identifier to each slice
            // eslint-disable-next-line no-unused-vars
            .on("click", function (_) {
                const clickedSliceId = d3.select(this).attr("data-slice-id")

                // Dim all slices except the clicked one
                slice
                    .select("path")
                    .transition()
                    .duration(200)
                    .attr("fill", (_, k) => {
                        return k.toString() === clickedSliceId ? String(color(k.toString())) : (d3.color(color(k.toString()))?.darker(0.3) as any) || "#ccc"
                    })
            })
