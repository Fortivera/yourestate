...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createDonutChart = (data: Property[]) => {
        ...
        const slice = svg.selectAll('.arc')
            ...
            .append('g')
            .classed('arc', true)
            .on('click', (event: any, d: d3.PieArcDatum<[string, number]>, i: number) => handleSliceClick(event, d, i));
    ...
    const handleSliceClick = (event: React.MouseEvent<SVGGElement, MouseEvent>, d: d3.PieArcDatum<[string, number]>, index: number) => {
