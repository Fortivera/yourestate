        ...
        <div className="relative inline-block text-left w-auto md:w-72">
            ...
            {isOpen && (
                <div className="origin-top-right absolute right-0 mt-2 w-full rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1">
                        {allOptions.map((propertyType: any, index: number) => (
                            <div key={index} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" onClick={() => handleOptionClick(propertyType)}>
                                {propertyType}
                            </div>
                        ))}
                    </div>
