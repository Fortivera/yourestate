...
export const D3DonutPieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[], theme: string) => {
        ...
        const legendSvg = d3.select(ref.current).append("svg").attr("width", 150).attr("height", 300).style("overflow-y", "scroll") // Make it scrollable

        // Append your legend items to the SVG
        const legend = legendSvg
            ...
            .attr("transform", (d, i) => `translate(${width - legendWidth}, ${i * 25})`) // Adjust this line
            .on("click", function (event, d) {
                activeIndex = activeIndex === d.index ? null : d.index
                updateChart()
            })
