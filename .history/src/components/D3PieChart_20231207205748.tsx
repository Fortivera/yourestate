...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[], theme: string) => {
        ...
        const legendSvg = d3.select(legendRef.current).attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet")
    ...
    return (
        <div className={`${theme === "light" ? "bg-slate-50" : "bg-[#515F73]"} w-full`}>
            <div className="max-w-[42.5rem] mx-auto p-2">
                <svg ref={ref} style={{ width: "20rem", height: "auto" }} />
                <div className="overflow-y-auto h-64 border-gray-100 border">
                    <svg ref={legendRef} style={{ width: "100%", height: "auto" }} />
                </div>
            </div>
        </div>
    )
