    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g.selectAll('g')
            ...
            .on('mouseover', function (this: SVGElement, d) {
                ...
                const currentIndex = slice.node()?.parentNode?.children.indexOf(slice.node());
                if (currentIndex !== undefined) {
                    d3.selectAll(`g:not(:nth-child(${currentIndex + 1})) path`)
            ...
            .on('mouseout', function () {
                ...
                const slice = d3.select(this);
                const originalColor = slice.attr('data-original-color'); // Retrieve the original color

                // Set the fill back to the original color only for the hovered slice
                slice.select('path').attr('fill', originalColor);

                // Set the fill back to the original color for other slices
                d3.selectAll('g path').attr('fill', (_, j) => String(color(j.toString())));
