            ...
            .on('click', function (_, d) {
                ...
                d3.select(this).select('path')
                    ...
                    .attr('fill', function (this: SVGPathElement, pathData, k) {
                        return k.toString() === clickedSliceId
                            ? (pathData.data.clicked ? String(color(k.toString())) : (d3.color(color(k.toString()))?.brighter(0.7) as any) || '#ccc')
                            : (d3.color(color(k.toString()))?.darker(0.3) as any) || '#ccc';
                    })
                    .each(function (this: SVGPathElement, pathData) {
                        // Update the clicked flag for each slice
                        const k = Number(d3.select(this.parentNode).attr('data-slice-id'));
