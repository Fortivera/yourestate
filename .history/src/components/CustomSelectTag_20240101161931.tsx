...
export const CustomDropdown = ({ options, theme }: any) => {
    ...
    return (
        <div className={"relative inline-block text-left w-auto md:w-72"}>
            ...
            {isOpen && (
                <div className={`${theme === "light" ? "bg-white text-black" : "bg-[#a6a9b3fd]  text-white"} origin-top-right absolute right-0 mt-2 w-full rounded-md shadow-lg  ring-1  ring-gray-100 ring-opacity-10 focus:outline-none`}>
                    <div className="py-1">
                        {options.map((userinput: any, index: any) => (
                            <option key={index} className={`${theme === "light" ? "bg-white text-black" : "bg-[#a6a9b3fd]  text-white"} block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 hover:border-rounded`} onClick={() => handleOptionClick(userinput)}>
                                {userinput}
                            </option>
                        ))}
                    </div>
