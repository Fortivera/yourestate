    ...
    const createPieChart = (data: Property[]) => {
        ...
        slice
            ...
            .on("click", function () {
                ...
                if (activeSlice === clickedSlice) {
                    ...
                    g.selectAll("path").transition().duration(50).style("opacity", 0.5)
                    d3.select(clickedSlice).transition().duration(50).style("opacity", 1)
        ...
        const legend = svg
            ...
            .on("click", function () {
                ...
                if (clickedLegend.classed("active")) {
                    // If the clicked legend is already active, reset opacity for all slices
                    slice.selectAll("path").transition().duration(200).style("opacity", 1)
                    clickedLegend.classed("active", false)
                } else {
                    // Otherwise, update the active legend and set opacity accordingly with a smooth transition
                    slice.selectAll("path").transition().duration(500).style("opacity", 0.5)
                    clickedLegend.transition().duration(500).style("opacity", 1)
                    clickedLegend.classed("active", true)
                }
            })
