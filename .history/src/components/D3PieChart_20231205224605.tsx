...
export const D3DonutChart: React.FC<Props> = ({ allProperties }: Props) => {
    const ref = useRef<SVGSVGElement>(null);
    const chartCreated = useRef(false);
    const [dimmedSlice, setDimmedSlice] = useState<number | null>(null);

    const handleSliceClick = useCallback(
        (event: React.MouseEvent<SVGGElement, MouseEvent>, d: d3.PieArcDatum<[string, number]>) => {
            const index = d.index!;
    ...
    const createDonutChart = useCallback((data: Property[]) => {
        ...
        const slice = svg.selectAll('.arc')
            ...
            .append('g')
            .classed('arc', true)
            .on('click', (event, d) => handleSliceClick(event, d));
    ...
    }, [allProperties, dimmedSlice, createDonutChart]);
