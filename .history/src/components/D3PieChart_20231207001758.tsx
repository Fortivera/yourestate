                ...
                if (activeSlice === clickedSlice) {
                    ...
                    legend.selectAll('text')
                        .style('opacity', (legendData, i, nodes) => {
                            const selectedNode = d3.select(nodes[i]) as unknown as d3.Selection<SVGTextElement, { index: number }, null, undefined>;
                            const isActive = selectedNode.datum()!.index === clickedIndex;
                            return isActive ? 1 : 0.5;
                        });

                    legend.selectAll('rect')
                        .style('opacity', (legendData, i, nodes) => {
                            const selectedNode = d3.select(nodes[i]) as unknown as d3.Selection<SVGRectElement, { index: number }, null, undefined>;
                            const isActive = selectedNode.datum()!.index === clickedIndex;
                            return isActive ? 1 : 0.5;
                        });
