...
const FloatingLabelInput: React.FC<FloatingLabelInputProps> = ({ defaultValue }) => {
    ...
    return (
        <div className={`flex flex-col py-2 relative w-full md:w-72`}>
            <label htmlFor="address" className={`absolute left-3 top-[2] transition-all duration-200 ${isFocused || value ? "text-sm top-0" : "text-base top-3"} ${theme === "light" ? "text-gray-700" : "text-gray-300"}`}>
                Address
            </label>
