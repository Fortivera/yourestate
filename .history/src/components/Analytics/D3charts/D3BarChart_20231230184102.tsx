...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const [svgWidth, setSvgWidth] = useState('0px'); // Initialize the width state

    useEffect(() => {
        if (allProperties && allProperties.length > 0) {
            const width = createBarChart(allProperties, theme); // Calculate the width
            setSvgWidth(`${width}px`); // Update the width state
        }
    }, [allProperties, theme]);
     ...
     const createBarChart = (data: Property[], theme: string) => {
        ...
        const dynamicWidth = barWidth * data.length;
