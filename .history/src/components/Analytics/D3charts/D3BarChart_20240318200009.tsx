...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createBarChart = (data: Property[], theme: string) => {
        ...
        svg.append("text")
            ...
            .attr("y", -10) // Center the label vertically
            .attr("x", 0 - (height / 2)) // Center the label vertically
            .attr("dy", "1em") // Adjust the position slightly right (when rotated)
            .style("text-anchor", "middle") // Center the text
            .text("Price ($USD/Month)") // The label text
            .style("fill", `${theme === "light" ? "black" : "white"}`);
