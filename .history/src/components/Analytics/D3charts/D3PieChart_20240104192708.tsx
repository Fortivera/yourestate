...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
  ...
  for (let i = baseLength; i < desiredLength; i++) {
    ...
    if (baseColor) {
      // Adjust hue and saturation while keeping lightness high
      baseColor.h = (baseColor.h + (i * 20)) % 360; // Adjust hue
      baseColor.s *= 0.9; // Slightly reduce saturation

      extendedScheme.push(baseColor.toString());
    }
  }
