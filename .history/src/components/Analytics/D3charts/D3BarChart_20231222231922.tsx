...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
  ...
  const createBarChart = (data: Property[], theme: string) => {
    ...
    // Y-axis
    svg.append("g").attr("transform", `translate(${margin.left},0)`).call(d3.axisLeft(yScale)).selectAll("path,line,text").style("stroke-width", "1px").style("font-weight", "normal").style("font-size", "1rem")

    const legend = svg.append("g").attr("class", "legend").attr("transform", `translate(100, 0)`).classed("cursor-pointer", true)
