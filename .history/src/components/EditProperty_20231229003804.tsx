...
export default function EditProperty({ property }: Props) {
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const cachedProperties: Property[] = queryClient.getQueryData(["allProperties"])
            if (cachedPropertiesroperties) {
                const updatedProperties = cachedProperties.map((p) => (p.id === updatedProperty.id ? updatedProperty : p))
