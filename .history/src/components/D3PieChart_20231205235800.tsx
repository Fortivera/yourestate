...
const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        function handleSliceClick(this: SVGGElement, d: d3.PieArcDatum<PieChartDatum>) {
            ...
            arcs.select('path')
                ...
                .duration(200)
                .attr('fill', (arcData) => {
                    if (isClicked || arcData.data.index === d.data.index) {
                        return color(arcData.data.data[0][0]);
                    } else {
                        return (d3.color(color(arcData.data.data[0][0]))?.darker(0.3) as any) || '#ccc';
                    }
                });
        }
    };
