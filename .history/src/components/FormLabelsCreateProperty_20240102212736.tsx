    ...
    return (
        <>
            ...
            {attributesHTML.map((entity) => {
                return (
                    <div key={entity.id} className="flex flex-col md:flex-row py-2 items-center">
                        <label className="pr-3 w-52" htmlFor={`${entity.label}`}>
                            {entity.label}
                        </label>
                        <input
                            className={`${theme === "light" ? "bg-white border-2 border-[#9da0a63d] text-black focus:ring-[#dadde195]" : "bg-[#767a8572] border-2 border-[#595c653d] text-white focus:ring-[#989898aa]"} leading-7 w-full md:w-72 rounded-lg p-[2px]  focus:outline-none focus:ring-[1px] `}
                            {...entity} 
                            required
