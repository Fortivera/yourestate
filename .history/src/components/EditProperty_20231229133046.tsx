...
export default function EditProperty({ property, allProperties: cachedProperties }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data: any, variables: Property) => {
      ...
      const updatedCachedProperties = cachedProperties!.map((cachedProperty) => (cachedProperty.id === updatedProperty.id ? updatedProperty : cachedProperty))
      queryClient.setQueryData(["allProperties"], updatedCachedProperties)
      console.log("Initial Fetch Data:", JSON.stringify(updatedCachedProperties, null, 2))
