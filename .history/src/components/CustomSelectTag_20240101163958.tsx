                ...
                <div className={`${theme === "light" ? "bg-white text-black" : "bg-[#595d69fd]  text-white"} origin-top-right absolute right-0 mt-2 w-full rounded-md shadow-lg  ring-1  ring-gray-100 ring-opacity-10 focus:outline-none`}>
                    <div className="py-1">
                        {options.map((userinput: any, index: any) => (
                            <option key={index} className={`${theme === "light" ? " text-black" : "hover:bg-[#898a90fd]  text-white"} block px-4 py-2 text-sm hover:rounded-sm`} onClick={() => handleOptionClick(userinput)}>
                                {userinput}
                            </option>
                        ))}
                    </div>
