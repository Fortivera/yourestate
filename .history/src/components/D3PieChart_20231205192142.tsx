    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g
            ...
            .on("click", function (_, j) {
                ...
                clickedSlice.classed("text-black", !isClicked).classed("fill-current", !isClicked).classed("filter-none", !isClicked).classed("filter-greyscale", !isClicked)

                // Update other slices to have a grey filter and dimmed color
                slice
                    .filter((_, k) => !isClicked && String(k) !== String(j))
                    .select("path")
