...
export const CustomDropdown = (options: any) => {
    ...
    return (
        <div className="relative inline-block text-left w-auto md:w-72">
            <div
                className={`${theme === "light" ? "bg-white border-2 border-[#9da0a63d] text-black focus:border-[#47444422]" : "bg-[#767a8572] border-2 border-[#595c653d] text-white focus:border-[#cf424291]"}
                ...
                <div className="origin-top-right absolute right-0 mt-2 w-full rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1">
                        {options.map((userinput: any, index: any) => (
                            <div key={index} className={`"block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900"`} onClick={() => handleOptionClick(userinput)}>
                                {userinput}
                            </div>
                        ))}
                    </div>
