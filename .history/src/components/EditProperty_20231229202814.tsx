...
export default function EditProperty({ property, allProperties: cachedProperties }: Props) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data: any, variables: Property) => {
            ...
            const updatedProperty = variables as Property
            console.log("data", data, "variables", updatedProperty)
            // Update the cached data
            const updatedCachedProperties = cachedProperties!.map((p: Property) => {
                if (p.id === Number(updatedProperty.id)) {
                    console.log("cached", p, "updated", updatedProperty)
                    // eslint-disable-next-line no-unused-vars
                    const { id, ...restOfUpdatedProps } = updatedProperty
                    console.log("restOfUpdatedProps", restOfUpdatedProps)
                    //   p.id = p.id as number
                    //   p.type = updatedProperty.type
                    //   p.address = updatedProperty.address
                    //   p.city = updatedProperty.city
                    //   p.suite = updatedProperty.suite
                    //   p.postalCode = updatedProperty.postalCode
                    //   p.province = updatedProperty.province
                    //   p.country = updatedProperty.country
                    //   p.tenant = updatedProperty.tenant
                    //   p.rent = updatedProperty.rent
                    //   p.surfaceArea = updatedProperty.surfaceArea
                    //   p.age = updatedProperty.age
                    //   p.electricity = updatedProperty.electricity
                    //   p.hydro = updatedProperty.hydro
                    //   p.gas = updatedProperty.gas
                    console.log("Updating property with id:", p.id, " to:", updatedProperty)
                    return { ...p, ...restOfUpdatedProps }
                }
