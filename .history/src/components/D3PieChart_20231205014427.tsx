        ...
        const slice = g
            ...
            .on("mouseover", function (_, i) {
                ...
                g.selectAll("path")
                    .transition()
                    .duration(200)
                    .attr("fill", (d, j) => (i === j ? color(i.toString()) : d3.color(color(j.toString()))!.darker(0.7)))

                // Brighten the hovered slice
                d3.select(this).select("path").transition().duration(200).attr("fill", color(i.toString()))
            })
            .on("mouseout", function (_, i) {
                // Restore color for all slices
                g.selectAll("path")
                    .transition()
                    .duration(200)
                    .attr("fill", (d, j) => color(j.toString()))
            })
