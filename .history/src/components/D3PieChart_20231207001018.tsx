            ...
            .on("click", function () {
                ...
                if (activeSlice === clickedSlice) {
                    ...
                    legend.selectAll("text").style("opacity", (legendData, i, nodes) => {
                        const isActive = (d3.select(nodes[i]) as d3.Selection<SVGTextElement, { index: number }, null, undefined>).datum().index === clickedIndex
                        return isActive ? 1 : 0.5
                    })

                    legend.selectAll("rect").style("opacity", (legendData, i, nodes) => {
                        const isActive = (d3.select(nodes[i]) as d3.Selection<SVGRectElement, { index: number }, null, undefined>).datum().index === clickedIndex
                        return isActive ? 1 : 0.5
                    })
