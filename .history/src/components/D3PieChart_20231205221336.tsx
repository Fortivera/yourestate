    ...
    const createPieChart = (data: Property[]) => {
        ...
        const slice = g.selectAll('g')
            ...
            .on('click', function (_, j) {
                ...
                const isDimmed = clickedSlice.classed('dimmed');

                // Remove 'dimmed' class from all slices
                slice.classed('dimmed', false);

                // Toggle the 'dimmed' class for the current slice
                clickedSlice.classed('dimmed', !isDimmed);

                // Update the fill color based on the 'dimmed' class
                slice.select('path')
                    .transition()
                    .duration(200)
                    .attr('fill', (_, k) => {
                        if (clickedSlice.classed('dimmed') && String(k) !== String(j)) {
