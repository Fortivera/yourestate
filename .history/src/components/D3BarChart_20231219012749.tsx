...
export const D3BarChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createBarChart = (data: Property[]) => {
        ...
        // Append the 'g' element for the x axis directly
        g.append('g')
            .attr('transform', `translate(0,${height})`)
            .call(d3.axisBottom(x0));

        // Append the 'g' element for the y axis directly
        g.append('g')
        ...
        // This function checks if a string is a key of EnergySums
        function isKeyOfEnergySums(key: any): key is keyof EnergySums {
            return key === 'electricity' || key === 'gas' || key === 'hydro';
        }

        // Then you would use this function before accessing the value
        const type = 'someStringFromSomewhere';
        if (isKeyOfEnergySums(type)) {
            // TypeScript now knows that type is a key of EnergySums
            const value = energySumsByCountry['someCountry'][type];
        }
        ...
        g.append('g')
            ...
            .selectAll('rect')
            .data(d => energyTypes.forEach((type) => {
                if (isKeyOfEnergySums(type)) {
                    // Now TypeScript knows that 'type' is definitely a key of EnergySums
                    svg.selectAll(`.${type}`)
                        .data(energyTuples)
                        .enter()
                        .append('rect')
                        .attr('x', ([country]) => (xScale(country) || 0) + x1(type)!)
                        .attr('y', ([, energySums]) => y(energySums[type]))
                        .enter().append('rect')
                        .attr('x', d => x1(d.key)!)
                        .attr('y', d => y(d.value))
