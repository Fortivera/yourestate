...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobilie Navbar */}
      <div className={`md:hidden ${menuOpen ? "flex" : "hidden"} relative w-full z-50`}>
        <div className={`flex flex-col  bg-gray-50 absolute right-0 border ${theme === "light" ? "bg-slate-200" : "navbarDark"}`}>
          <ThemeToggle />
          <Link href={"dashboard/createproperty"}>
            <div className=" blur-sm h-10 flex items-center px-3">
              <SvgAddProperty className="m-1" strokeWidth={1.5} />
              <div>Add Property</div>
            </div>
          </Link>
