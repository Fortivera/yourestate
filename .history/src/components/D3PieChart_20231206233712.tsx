...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    const createPieChart = (data: Property[]) => {
        ...
        const updateOpacity = () => {
            console.log("Updating Opacity:", clickedIndex)
            slice.selectAll("path").style("opacity", (d: any) => (d.index === clickedIndex ? 1 : 0.5))

            legend.selectAll("text").style("opacity", (d: any) => (d.index === clickedIndex ? 1 : 0.5))
        }
