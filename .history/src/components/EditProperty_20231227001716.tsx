...
export default function EditProperty(propertyid: number) {
  const { updatePropertyZustand, deletePropertyZustand, allProperties } = usePropertyStore()
  const router = useRouter()
  const property = filterId(allProperties, propertyid) as Property
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      updatePropertyZustand(property.id, property)
      // router.refresh()
  ...
  async function handleDelete(event: any) {
    ...
    deletePropertyZustand(property.id)
    // router.refresh()
    router.replace("/dashboard")
  }
