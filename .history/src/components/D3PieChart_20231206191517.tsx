...
export const D3PieChart: React.FC<Props> = ({ allProperties }: Props) => {
    ...
    useEffect(() => {
        ...
        // Event listener for window resize to update viewBox
        const handleResize = () => {
            updateViewBox()
        }
    ...
    const createPieChart = (data: Property[]) => {
        ...
        // Update the viewBox and preserveAspectRatio attributes for responsiveness
        const updateViewBox = () => {
            const { width, height } = ref.current.getBoundingClientRect()
            svg.attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet")
        }

        // Initial call to set viewBox based on the initial size of the container
        updateViewBox()
        ...
        const arcGenerator = d3
            ...
            .innerRadius(Math.min(svg.node()!.clientWidth, svg.node()!.clientHeight) / 4)
            .outerRadius(Math.min(svg.node()!.clientWidth, svg.node()!.clientHeight) / 2)
        ...
        slice
            ...
            .on("click", function () {
                ...
                if (activeSlice === clickedSlice) {
                    ...
                    g.selectAll("path").transition().duration(200).style("opacity", 1)
                    activeSlice = null
                } else {
        ...
        const legend = svg
            .append("g")
            .attr("transform", `translate(${svg.node()!.clientWidth}, 50)`)
            .selectAll("g")
            .data(pieData)
            .enter()
            .append("g")
            ...
            .on("click", function (event, d) {
                ...
                if (clickedSlice.attr("opacity") === "1") {
                    // If the clicked slice is the same as the active one, reset opacity for all slices
                    slice.selectAll("path").transition().duration(200).style("opacity", 1)
                } else {
                    // Otherwise, update the active slice and set opacity accordingly with a smooth transition
