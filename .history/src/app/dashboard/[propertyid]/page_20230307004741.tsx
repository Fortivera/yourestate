...
export default async function ShowProperty({ params: { propertyid } }: Params) {
    ...
    async function handlePut(event: Formeve) {
    ...
    return (
        <Modal>
            <form method="post" id="property-form" onSubmit={handlePut}>
                ...
                <div className="w-[30rem] py-2 px-8 flex flex-col mx-auto border-2 border-gray-400 z-1">
                    ...
                    <div className="my-5 ">
                        <div className="flex flexcol items-center justify-center gap-20">
                            <button onSubmit={deleteProperty} className="w-28 rounded-md py-1 bg-red-300 shadow-md hover:bg-red-400">
...
async function updateProperty(userInput: number) {
    const url = `${process.env.NEXT_PUBLIC_SERVER_URL}${process.env.NEXT_PUBLIC_PROPERTY_ENDPOINT}${userInput}`
    console.log(url)
    try {
        const response = await fetch(url, {
