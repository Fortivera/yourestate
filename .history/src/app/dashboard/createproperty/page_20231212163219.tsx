          ...
          {formLables.map((entity) => {
            return (
              <div key={entity.id} className="flex flex-col md:flex-row py-2 ">
                ...
                <input className={`${theme === "light" ? "bg-white" : inputColour} leading-7 w-auto md:w-72 border-b-2`} id="" type={`${entity.type}`} name={`${entity.name}`} aria-label={`${entity.ariaLabel}`} placeholder={`${entity.placeholder}`} onKeyDown="fsdf" />
              </div>
            )
          })}
          <div className="my-5 ">
